#!/usr/bin/env python
"""Пересчёт метрик оценки агентов из сохранённых ответов (runs/eval_answers_*.jsonl).

Зачем: модели дрейфуют по языку маркеров — «### Предварительный диагноз» →
«### Preliminary diagnosis» → «### Answer» (USMLE-виньетки). Вместо повторных
GPU-прогонов метрики считаются офлайн по полным ответам с расширенными
регулярками, единообразно для всех агентов и секций.

Выход: runs/FINAL_METRICS.md — сводная таблица (формат, MCQ с покрытием,
чистота EN) по 14 агентам; используется в финальном отчёте.
"""
