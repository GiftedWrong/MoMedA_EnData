#!/usr/bin/env python
"""Данные мастер-агента (med-chief-3b): синтез итогового диагноза из
предварительных на DDXPlus.

Пара: вход — виньетка (EN, evidences) + «мнения специалистов» (топ-k гипотез из
DIFFERENTIAL_DIAGNOSIS; истинная патология с вероятностью ~0.5 добавляется в
список, если её там нет — как в реальности роутер/агенты иногда правы);
выход — ### Рассуждение (шаблонный синтез: соответствие улик) +
### Итоговый диагноз (PATHOLOGY).

Запуск:
  .venv/bin/python scripts/prepare_chief.py [--n 15000]
Выход: data/chief/{train,val,test}.jsonl
"""
