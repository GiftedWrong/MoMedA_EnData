#!/usr/bin/env python
"""Сквозной e2e-бенчмарк инференс-конвейера: RU-жалоба → /api/case → RU-ответ.

Фаза 1 (offline, кэшируется): 200 жалоб из DDXPlus (evidences, жалобный профиль
прод-входа; золото специальности через DDX_DISEASE_RULES) переводятся в RU
загруженным один раз base Qwen — data/processed/e2e_ru_complaints.jsonl.

Фаза 2: жалобы (отсортированные по золотой специальности — меньше перезагрузок
агентов) прогоняются через POST /api/case; метрики: точность маршрутизации
end-to-end, наличие диагноза в RU-ответе, чистота русского (CJK/латиница),
латентность по шагам; сэмплы для просмотра.

Запуск (сервер должен работать на 127.0.0.1:8010; для скорости — с
MOMEDA_TRANSLATOR_TTL=600):
  .venv/bin/python scripts/benchmark_e2e.py [--n 200] [--port 8010]
"""
